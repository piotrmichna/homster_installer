...
function install_prog(){
    ...
    dpkg -s $1 &> /dev/null
    if [ $? -eq 0 ] ; then
        echo -e "${GREEN}---> $1 jest już zainstalowane $NC" |& tee -a ${HOME_DIR}/${LOG_FILENAME}_$currentDate.log &> /dev/null
        TERM=ansi whiptail --title "- $1 -" --infobox "$1 był już zainstalowany" 8 70
    else
        echo -e "${GREEN}--->INSTALL $1 $NC" |& tee -a ${HOME_DIR}/${LOG_FILENAME}_$currentDate.log &> /dev/null
